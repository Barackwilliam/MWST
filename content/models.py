...
class Announcement(Bilingual, TimeStamped):
    ...
    # --- Uidhinishaji ---
    # Afisa au Mratibu anaweza kuandaa tangazo, lakini Msimamizi pekee ndiye
    # anayeliruhusu kuonekana kwa umma. Bila hivyo mtu yeyote mwenye akaunti
    # ya ofisi angeweza kuchapisha chochote kwa jina la MUWESTA.
    status = models.CharField(
        _("Hali"), max_length=12, default="draft",
        choices=[("draft", _("Rasimu")), ("pending", _("Inasubiri idhini")),
                 ("approved", _("Imeidhinishwa")), ("rejected", _("Imekataliwa"))])
    is_urgent = models.BooleanField(
        _("Ni la dharura"), default=False,
        help_text=_("Linang'aa juu ya ukurasa wa mwanzo."))
    created_by = models.ForeignKey(
        "accounts.User", null=True, blank=True, on_delete=models.SET_NULL,
        related_name="announcements_created", verbose_name=_("Ameandaa"))
    approved_by = models.ForeignKey(
        "accounts.User", null=True, blank=True, on_delete=models.SET_NULL,
        related_name="announcements_approved", verbose_name=_("Ameidhinisha"))
    approved_at = models.DateTimeField(_("Muda wa idhini"), null=True, blank=True)
    review_note = models.CharField(_("Sababu"), max_length=200, blank=True)

    class Meta:
        ordering = ["-is_urgent", "-published_on", "-id"]
    ...
    @property
    def is_live(self):
        """Linaonekana kwa umma tu likiwa limeidhinishwa NA linatumika."""
    ...
    def reject(self, user, reason=""):
        ...
        self.approved_by = user
        self.approved_at = tz.now()
        self.review_note = reason[:200]
        self.save(update_fields=["status", "approved_by", "approved_at", "review_note"])
...
class Milestone(Bilingual):
    ...
    class Meta:
        ...
        verbose_name_plural = _("Historia ya MUWESTA")
...
class Notification(TimeStamped):
    ...
    member = models.ForeignKey("members.Member", null=True, blank=True,
                               on_delete=models.CASCADE, related_name="notifications")
    #: Kitambulisho cha kuzuia arifa moja kutumwa mara mbili. Amri
    #: zinazoendeshwa kila siku (mfano arifa za muda kuisha) hutumia hii
    #: — bila yake, mtu angepokea arifa ile ile kila asubuhi.
    ...
    @classmethod
    def once(cls, key, **fields):
        """
        Unda arifa kama haijawahi kuundwa kwa `key` hii.

        Hurudisha arifa iliyoundwa, au `None` kama tayari ilikuwepo.
        Ni hii inayoruhusu amri ya arifa kuendeshwa kila siku bila
        kumzidishia mtu ujumbe uleule.
        """
        if not key:
            raise ValueError("Notification.once inahitaji `key`")
        if cls.objects.filter(key=key).exists():
            return None
        return cls.objects.create(key=key, **fields)
