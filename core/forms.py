...
class ApplicationForm(BootstrapMixin, forms.ModelForm):
    class Meta:
        model = Application
        fields = ["full_name", "gender", "date_of_birth", "id_type", "national_id", "nationality",
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Aina ya kitambulisho haipaswi kuwa na chaguo tupu — NIDA ndiyo
        # chaguo-msingi, lakini mtu anaweza kubadilisha.
        if "id_type" in self.fields:
            from members.models import ID_TYPES
            self.fields["id_type"].label = _("Aina ya Kitambulisho")
            self.fields["id_type"].required = False
            # Django huongeza chaguo tupu "---------" kwenye sehemu zisizo
            # za lazima. Hapa halina maana — NIDA ndiyo chaguo-msingi.
            self.fields["id_type"].choices = ID_TYPES
...
class ContactForm(BootstrapMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Thamani inayohifadhiwa inabaki Kiswahili (ndiyo inayosomwa na
        # afisa), lakini LEBO inatafsiriwa — vinginevyo mtu wa Kiingereza
        # angeona "Uanachama" kwenye fomu ya Kiingereza.
        self.fields["subject"] = forms.ChoiceField(
            label=_("Mada"), choices=[(s, gettext(s)) for s in self.SUBJECTS])
        self._style()
...
# ===========================================================================
class PublicDonationForm(BootstrapMixin, forms.Form):
    """
    Fomu ya michango ya umma — muundo wa mockup ya MICHANGO.

    Si ModelForm: sehemu nyingi (aina ya mchango, kurudia, fedha, mtoa
    huduma) zinatoka `core/data/giving.py`, si moja kwa moja kwenye modeli.
    `save()` ndiyo inayotengeneza `Contribution`.

    HAKUNA sehemu ya namba ya kadi wala CVV hapa, na haitakuwepo. Kadi
    ...
    email = forms.EmailField(label=_("Barua pepe (hiari)"), required=False)
    region = forms.CharField(label=_("Mkoa"), max_length=80, required=False)
    message = forms.CharField(label=_("Ujumbe (hiari)"), max_length=200, required=False,
    ...
    def summary(self):
        """
        Muhtasari wa kile atakacholipa.

        `amount` ni cha MWEZI MMOJA. Akichagua miezi kadhaa, jumla
        inazidishwa kisha punguzo linapunguzwa — `recurrence_total`
        ndiyo yenye hesabu, ili fomu, JavaScript na risiti visome
        chanzo kimoja.
        """
        data = self.cleaned_data
        code = data["currency"]
        rec = giving.recurrence(data.get("recurrence") or "once")
        per_month = data["amount"]
        total = giving.recurrence_total(per_month, rec["key"])
        return {
            "purpose": giving.purpose(data["purpose"]),
            "recurrence": rec,
            "amount": per_month,
            "months": rec["months"],
            "discount": rec["discount"],
            "total": total,
            "currency": giving.currency(code),
            "tzs": giving.to_tzs(total, code),
            "fee": 0,
        }

    def save(self):
        """Tengeneza mchango. Unabaki `pending` hadi afisa athibitishe."""
        from finance.models import Contribution, Fund, PaymentStatus

        data = self.cleaned_data
        spec = giving.purpose(data["purpose"])
        fund = (Fund.objects.filter(code=spec["fund"]).first()
                or Fund.objects.order_by("order").first())

        # Kiasi kinachohifadhiwa ni JUMLA (miezi × punguzo), si cha mwezi
        # mmoja. `entered_amount` inabaki kile alichoandika, kwa ajili ya
        # risiti inayoeleza "TZS 10,000 kwa mwezi × miezi 6".
...
class MembershipPaymentForm(BootstrapMixin, forms.Form):
    """
    Kulipa ada ya uanachama (muundo wa mockup ya LIPA ADA).

    Bei haichukuliwi kutoka kwa mtumiaji — inahesabiwa upya hapa kutoka
    `Category.monthly_fee` na kipindi alichochagua. Hivyo mtu hawezi
    kubadilisha kiasi kwenye kivinjari akalipa kidogo.

    Kama ilivyo kwenye michango, HAKUNA namba ya kadi wala CVV hapa.
    """
    package = forms.ChoiceField(label=_("Kifurushi"))
    #: "ada" = mchango wa mwezi (haugusi muda), "uhuisho" = kipindi
    #: kingine cha miaka mitatu. Tofauti hii inaamua kama malipo
    #: yatasogeza tarehe ya kuisha au la.
    pay_kind = forms.ChoiceField(label=_("Aina ya malipo"), required=False)
    #: Miezi anayotaka kulipia. Namba yoyote kati ya MIN na MAX —
    #: si orodha ya vifurushi. Mtu anayeweza kulipa mwezi mmoja tu
    #: asizuiwe, na anayetaka kulipia miaka miwili aweze.
    months = forms.IntegerField(
        label=_("Miezi ya kuchangia"), required=False,
        min_value=giving.MIN_MONTHS, max_value=giving.MAX_MONTHS,
        error_messages={
            "min_value": _("Kiwango cha chini ni mwezi mmoja."),
            "max_value": _("Kiwango cha juu ni miezi %(limit_value)s."),
            "invalid": _("Weka idadi ya miezi kwa namba."),
        })
    payer_type = forms.ChoiceField(label=_("Aina ya mlipaji"))
    include_registration = forms.BooleanField(
        label=_("Ongeza ada ya usajili (mwanachama mpya)"), required=False)

    full_name = forms.CharField(label=_("Jina kamili"), max_length=160)
    email = forms.EmailField(label=_("Barua pepe"), required=False)
    phone = forms.CharField(label=_("Namba ya simu"), max_length=24)
    membership_no = forms.CharField(label=_("Namba ya uanachama"), max_length=40,
                                    required=False)
    national_id = forms.CharField(label=_("Namba ya kitambulisho"), max_length=40,
                                  required=False)
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        #: Hujazwa na `clean()` — ndipo malipo yanapojua yanamhusu nani.
        self.application = None
        self.member = None
        cats = Category.objects.filter(is_special=False)
        self._cats = {c.code: c for c in cats}
        self.fields["package"].choices = [(c.code, c.name) for c in cats]
        self.fields["payer_type"].choices = [(p["key"], p["name"]) for p in giving.PAYER_TYPES]
        self.fields["pay_kind"].choices = [(k["key"], k["name"]) for k in giving.PAY_KINDS]
        self.fields["provider"].choices = [(p["key"], p["name"]) for p in giving.PROVIDERS]
        self._style()
    ...
    def clean(self):
        ...
        if code and code not in self._cats:
            self.add_error("package", _("Chagua kifurushi sahihi."))

        # Ada lazima ifahamike inamhusu nani. Bila hii, mtu angelipa,
        # risiti ingetoka, lakini rekodi yake isingesogea hata kidogo.
        payer = data.get("payer_type")
        if payer == "new":
            self._resolve_application(data)
        elif payer == "member":
            self._resolve_member(data)

        # Mwombaji mpya hulipa usajili tu — hawezi "kuhuisha" kitu
        # ambacho hakijaanza. Kuhuisha ni kwa mwanachama pekee.
        if payer == "new":
            data["pay_kind"] = "ada"
        elif not data.get("pay_kind"):
            data["pay_kind"] = "ada"

        # Michango ya mwezi inahitaji idadi ya miezi; kuhuisha haihitaji
        # kwa sababu kipindi ni kilekile cha miaka mitatu daima.
        if data["pay_kind"] == "ada":
            months = data.get("months")
            if months in (None, ""):
                self.add_error("months", _("Weka idadi ya miezi unayochangia."))
        return data

    def _resolve_application(self, data):
        from members.models import Application, ApplicationStatus

        ref = (data.get("membership_no") or "").strip()
        if not ref:
            self.add_error("membership_no", _(
                "Weka namba ya ombi lako (mfano APP/MUWESTA/2026/0001). "
                "Utaipata kwenye ujumbe uliopokea baada ya kujiunga."))
            return
        app = Application.objects.filter(reference__iexact=ref).first()
        if app is None:
            self.add_error("membership_no", _("Hakuna ombi lenye namba hiyo."))
            return
        if app.member_id:
            self.add_error("membership_no", _(
                "Ombi hili tayari limekamilika. Tumia namba yako ya "
                "uanachama badala ya namba ya ombi."))
            return
        if app.status != ApplicationStatus.AWAITING_PAYMENT:
            self.add_error("membership_no", _(
                "Ombi hili bado halijahakikiwa na afisa. Utapigiwa simu "
                "litakapokuwa tayari kulipiwa."))
            return
        # Kifurushi kinatoka kwenye ombi, si kwenye fomu — mtu asijilipie
        # daraja la chini kuliko alilokubaliwa.
        self.application = app
        data["package"] = app.category.code
        data["include_registration"] = True

    def _resolve_member(self, data):
        from members.models import Member

        no = (data.get("membership_no") or "").strip()
        if not no:
            self.add_error("membership_no", _("Weka namba yako ya uanachama."))
            return
        member = Member.objects.filter(membership_no__iexact=no).first()
        if member is None:
            self.add_error("membership_no", _("Hakuna mwanachama mwenye namba hiyo."))
            return
        self.member = member
        data["package"] = member.category.code
        data["include_registration"] = False

    def totals(self):
        """Hesabu ya mwisho — inatoka kwenye database, si kwenye fomu."""
        data = self.cleaned_data
        cat = self._cats[data["package"]]
        kind = data.get("pay_kind") or "ada"
        registration = int(cat.registration_fee) if data.get("include_registration") else 0

        if kind == "uhuisho":
            # Kuhuisha kunagharimu ada ya usajili — ni kuanzisha kipindi
            # kipya, si kununua miezi. Hakuna miezi wala punguzo hapa.
            fee = int(cat.registration_fee or 0)
            return {"category": cat, "kind": kind, "months": 0, "fee": fee,
                    "discount": 0, "monthly": int(cat.monthly_fee or 0),
                    "registration": 0, "years": giving.TERM_YEARS,
                    "total": fee}

        months = giving.clamp_months(data.get("months"))
        fee = giving.months_price(cat.monthly_fee, months)
        return {"category": cat, "kind": kind, "months": months, "fee": fee,
                "discount": giving.discount_for(months),
                "monthly": int(cat.monthly_fee or 0),
