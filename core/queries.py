...
CAT_COLOURS = {"Bronze": C["orange"], "Silver": C["slate"], "Gold": C["gold"],
               "Platinum": C["purple"], "Tanzanite": C["navy"]}
...
def _recent_activity(limit=5):
    ...
    for m in Member.objects.order_by("-created_at")[:2]:
        items.append({"title": "Mwanachama mpya amejiunga", "sub": f"Mkoa wa {m.region.name if m.region else '—'}",
    ...
    for a in Application.objects.order_by("-created_at")[:1]:
        items.append({"title": "Maombi mapya ya uanachama",
                      "sub": f"Mkoa wa {a.region.name if a.region else '—'}",
    ...
    for t in PointTransaction.objects.order_by("-created_at")[:1]:
        items.append({"title": "Pointi zimeongezwa",
                      "sub": f"{num(t.points)} Pointi kwa {t.member.full_name}",
...
def superadmin(year=None):
    ...
    return {
        ...
        "announcements": [{"title": a.tx("title"), "icon": a.icon, "tint": a.tint,
                          ...
                          for a in Announcement.objects.filter(is_active=True)[:3]],
    }
...
def member_dashboard(member):
    ...
    from programs.models import Reward
    nxt = Reward.objects.filter(points_required__gt=points, is_active=True).first()
    remaining = (nxt.points_required - points) if nxt else 0
    progress = round(points / nxt.points_required * 100) if nxt else 100
    ...
    return {
        ...
        "kpis": [
            {"label": "Hali ya Uanachama", "value": member.get_status_display(),
             "icon": "check-circle", "tint": "green", "note": "Uanachama wako ni hai"},
            {"label": "Muda wa Uanachama",
             "value": member.expires_on.strftime("%d %b %Y") if member.expires_on else "—",
             "icon": "calendar", "tint": "navy", "money": True, "note": "Uanachama unamalizika"},
            {"label": "Pointi Zako", "value": num(points), "icon": "star", "tint": "gold",
             "note": "Jumla ya pointi"},
            {"label": "Ada ya Mwezi", "value": tzs(member.category.monthly_fee), "icon": "wallet",
        ...
        "points": {"current": num(points), "earned_this_month": f"+{num(month_points)}",
                   "level": member.category.name, "next_reward_remaining": num(remaining),
...
def _next_due(member):
    last = confirmed(member.payments.all()).order_by("-paid_at").first()
    base = last.paid_at.date() if last else member.joined_on
...
def public_uanachama():
    ...
    return {
        "hero": {"eyebrow": "Uanachama", "scene": "jamii",
                 ...
                 "text": "Uanachama wa MWST unakupa fursa ya kushiriki katika huduma za "
        ...
        "steps": [
            ...
            {"n": 2, "title": "Chagua Aina", "icon": "star",
             "text": "Chagua aina ya uanachama inayokufaa: Bronze hadi Platinum."},
...
def public_huduma():
    ...
    return {
        "hero": {"eyebrow": "Huduma Zetu", "scene": "jamii",
                 ...
                 "text": "Tunatoa huduma katika maeneo makuu yanayolenga kuinua hali ya "
...
def public_jiunge():
    u = public_uanachama()
    return {"hero": {"eyebrow": "Usajili", "scene": "elimu", "title": "Jiunge na MWST Leo",
...
def member_payments(member):
    ...
    due = member.category.monthly_fee * 12
    return {
        "kpis": [
            ...
            {"label": "Lengo la Mwaka", "value": tzs(due), "icon": "target", "tint": "gold",
...
def member_points(member):
    from programs.models import Reward
    balance = PointTransaction.balance(member)
    ...
    for r in Reward.objects.filter(is_active=True):
        rewards.append({
            ...
            "pct": min(round(balance / r.points_required * 100), 100) if r.points_required else 100,
            "reached": balance >= r.points_required,
            "remaining": num(max(r.points_required - balance, 0)),
        })
    return {
        "kpis": [
            {"label": "Pointi Zangu", "value": num(balance), "icon": "star", "tint": "gold"},
            ...
            {"label": "Daraja Langu", "value": member.category.tx("name"),
             "icon": "id-card", "tint": "navy"},
            {"label": "Miamala", "value": num(txns.count()), "icon": "receipt", "tint": "teal"},
        ],
        "rows": [{
