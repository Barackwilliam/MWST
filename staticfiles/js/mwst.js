...
/* ==========================================================================
   AINA YA UANACHAMA — kadi au orodha kunjuzi
   Vyote vina `name="category"`, kwa hiyo kimoja LAZIMA kizimwe —
   sehemu iliyozimwa haitumwi na fomu. Bila hii, fomu ingetuma
   thamani mbili na ile ya mwisho ndiyo ingeshinda.
   ========================================================================== */
(function () {
  "use strict";
  var sel = document.querySelector("[data-catsel]");
  var cards = document.querySelector("[data-catcards]");
  if (!sel || !cards) return;

  var radios = cards.querySelectorAll("input[type=radio]");
  var narrow = window.matchMedia("(max-width: 640px)");

  function sync() {
    var small = narrow.matches;
    sel.disabled = !small;
    for (var i = 0; i < radios.length; i++) radios[i].disabled = small;
  }

  /* Chaguo lifuatane pande zote mbili — mtu akibadilisha upana wa
     dirisha, asikute amechagua kitu tofauti. */
  cards.addEventListener("change", function (e) {
    if (e.target.type === "radio") {
      sel.value = e.target.value;
      var all = cards.querySelectorAll(".catcard");
      for (var i = 0; i < all.length; i++) all[i].classList.remove("is-on");
      e.target.closest(".catcard").classList.add("is-on");
    }
  });

  sel.addEventListener("change", function () {
    for (var i = 0; i < radios.length; i++) {
      if (radios[i].value === sel.value) {
        radios[i].checked = true;
        var all = cards.querySelectorAll(".catcard");
        for (var k = 0; k < all.length; k++) all[k].classList.remove("is-on");
        radios[i].closest(".catcard").classList.add("is-on");
      }
    }
  });

  narrow.addEventListener ? narrow.addEventListener("change", sync)
